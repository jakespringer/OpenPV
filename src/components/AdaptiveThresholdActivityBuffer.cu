...
#include "AdaptiveThresholdActivityBuffer.kpp"

namespace PV {

void AdaptiveThresholdActivityBuffer::runKernel() {
   ...
   PVCuda::applyThresholdsAdaptiveThresholdActivityBufferOnGPU<<<currGridSize, currBlockSize, 0, cudaStream>>>(
         nbatch,
         numNeurons,
         loc->nx,
         loc->ny,
         ...
         loc->halo.up,
         mNumVertices,
         (float const *)mCudaThresholds->getPointer(),
